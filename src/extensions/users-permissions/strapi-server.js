...
const strapiServer = ({ strapi }) => {
    return {
      async register({ strapi }) {
        const extensionService = strapi.service('plugin::users-permissions.auth');
  
        const extendedForgotPassword = async (ctx) => {
        ...
        // 設定新的 forgotPassword 方法
        if (extensionService) {
          extensionService.forgotPassword = extendedForgotPassword;
        }
      },
